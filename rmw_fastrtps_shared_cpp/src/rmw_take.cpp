  ...
  rmw_subscription_t * subscription)
{
  ...
  if (nullptr != xcdr_ts) {
    // Advertise loan capability whenever the typesupport can back it.
    // A size threshold here only denied loans where measurement shows they
    // match (small payloads) or beat (large payloads) copy takes.
