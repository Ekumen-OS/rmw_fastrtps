  ...
  bool keyed)
{
  ...
  if (nullptr == ts_impl) {
    ...
    if (!fastdds_type) {
      constexpr bool kAssumeBounded = true;  // always bounded on the receiving end
      auto tsupport = new (std::nothrow) rmw_fastrtps_shared_cpp::XcdrTypeSupport(
        type_supports, constraints, topic_type_name, kAssumeBounded);
      if (!tsupport) {
  ...
  info->topic_ = participant_info->find_or_create_topic(
    topic_name_mangled, topic_type_name, topic_qos, info->subscription_event_);
  if (!info->topic_) {
    RMW_SET_ERROR_MSG("create_subscription() failed to create topic");
    return nullptr;
  }
